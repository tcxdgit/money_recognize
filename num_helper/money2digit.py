...
class trans_money():
    ...
    def fact(self, a, m_str):
        if a < 0:
            ...
            if self.mon[a] in m_str:
                ...
                if self.mon[a] == '万':
                    # 判断左边字符串是不是纯数字
                    if '一' not in m_left and '二' not in m_left and '两' not in m_left and '三' not in m_left and '四' not in m_left and '五' not in m_left and '六' not in m_left and '七' not in m_left and '八' not in m_left and '九' not in m_left and '十' not in m_left and '百' not in m_left and '千' not in m_left:
                        self.func4(a, m_left)
                    else:
                        self.fact(2, m_left)
                else:
    ...
    def mainfunc(self, m_str):
        ...
        if '块钱' in sentence:
            sentence = sentence.replace('块钱', '块')
        if '元钱' in sentence:
            sentence = sentence.replace('元钱', '元')
        if '毛钱' in sentence:
            sentence = sentence.replace('毛钱', '毛')
        if '角钱' in sentence:
            sentence = sentence.replace('角钱', '角')
        if '分钱' in m_str:
            sentence = sentence.replace('分钱', '分')

        num = 4
        liststr = list(sentence)
        while '加' in liststr:
            liststr.remove('加')
